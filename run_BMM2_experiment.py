...
#    n = int(sys.argv[1])
#    outliers = str_to_bool(sys.argv[2])
#    gamma = float(sys.argv[3])
    simulation_number = 0
    
    all_n = [5000]
    all_outliers = [True]
    all_gamma = [1.0, 1.01, 1.001]
    ...
    my_setting = all_settings[simulation_number]
    n, outliers, gamma = my_setting[0], my_setting[1], my_setting[2]
    ...
    model_misspecification = outliers
    
    # hard code loss type & its hyperpar
    if gamma != 1.0:
        losstype="gamma"
        losshyperparameter = gamma
    else:
        losstype="log"
        losshyperparameter=1.0

    # path to save stuff
    base_path = "/Users/jeremiasknoblauch/Documents/OxWaSP/GVI/JASA/code/experiments_FIXED_DATASET/BMM2"
    ...
    spec_string = ("/" + misspec_dir + "/n=" + str(n) + "/d=" + str(d) +
                   "/losstype=" + losstype + "_param=" + str(losshyperparameter))
